...
class AuthConfig(dict):
    ...
    def parse_auth(cls, entries, raise_on_error=False):
        ...
        for registry, entry in entries.items():
            if not isinstance(entry, dict):
                log.debug("Config entry for key {} is not auth config".format(registry))
                ...
                if raise_on_error:
                    raise errors.InvalidConfigFile(
                        "Invalid configuration for registry {}".format(registry)
                    )
                return {}
            if "identitytoken" in entry:
                log.debug(
                    "Found an IdentityToken entry for registry {}".format(registry)
                )
    ...
    def _resolve_authconfig_credstore(self, registry, credstore_name):
        ...
        try:
            ...
            raise errors.DockerException(f"Credentials store error: {repr(e)}")
