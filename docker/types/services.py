...
class Mount(dict):
    ...
    def __init__(
        ...
        if type not in ("bind", "volume", "tmpfs", "npipe"):
            raise errors.InvalidArgument(f'Unsupported mount type: "{type}"')
        self["Type"] = type
        self["ReadOnly"] = read_only
        ...
        if type == "bind":
            ...
            if tmpfs_mode:
                if not isinstance(tmpfs_mode, int):
                    raise errors.InvalidArgument("tmpfs_mode must be an integer")
    ...
    def parse_mount_string(cls, string):
        ...
        if len(parts) == 1:
            ...
            if source.startswith("/") or (
                IS_WINDOWS_PLATFORM and source[0].isalpha() and source[1] == ":"
            ):
...
class Resources(dict):
    ...
    def __init__(
        ...
        if generic_resources is not None:
            reservation["GenericResources"] = _convert_generic_resources_dict(
                generic_resources
            )
        if limits:
            self["Limits"] = limits
...
class RestartPolicy(dict):
    ...
    def __init__(
        ...
        if condition not in self.condition_types._values:
            raise TypeError(f"Invalid RestartPolicy condition {condition}")
...
class ServiceMode(dict):
    ...
    def __init__(self, mode, replicas=None):
        if mode not in ("replicated", "global"):
            raise errors.InvalidArgument('mode must be either "replicated" or "global"')
...
class SecretReference(dict):
    ...
    def __init__(
        self, secret_id, secret_name, filename=None, uid=None, gid=None, mode=0o444
    ):
...
class ConfigReference(dict):
    ...
    def __init__(
        self, config_id, config_name, filename=None, uid=None, gid=None, mode=0o444
    ):
...
class Privileges(dict):
    ...
    def __init__(
        self,
        credentialspec_file=None,
        credentialspec_registry=None,
        selinux_disable=None,
        selinux_user=None,
        selinux_role=None,
        selinux_type=None,
        selinux_level=None,
