...
class LogConfig(DictType):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__({"Type": log_driver_type, "Config": config})
    ...
    def __init__(
        ...
        if mem_swappiness is not None:
            if not isinstance(mem_swappiness, int):
                raise host_config_type_error("mem_swappiness", mem_swappiness, "int")
        ...
        if oom_score_adj:
            ...
            if not isinstance(oom_score_adj, int):
                raise host_config_type_error("oom_score_adj", oom_score_adj, "int")
        ...
        if restart_policy:
            if not isinstance(restart_policy, dict):
                raise host_config_type_error("restart_policy", restart_policy, "dict")
        ...
        if log_config is not None:
            if not isinstance(log_config, LogConfig):
                if not isinstance(log_config, dict):
                    raise host_config_type_error("log_config", log_config, "LogConfig")
        ...
        if cpuset_mems:
            if not isinstance(cpuset_mems, str):
                raise host_config_type_error("cpuset_mems", cpuset_mems, "str")
        ...
        if cpu_rt_period:
            ...
            if not isinstance(cpu_rt_period, int):
                raise host_config_type_error("cpu_rt_period", cpu_rt_period, "int")
        ...
        if cpu_rt_runtime:
            ...
            if not isinstance(cpu_rt_runtime, int):
                raise host_config_type_error("cpu_rt_runtime", cpu_rt_runtime, "int")
        ...
        if device_read_bps:
            if not isinstance(device_read_bps, list):
                raise host_config_type_error("device_read_bps", device_read_bps, "list")
            if version_lt(version, "1.22"):
        ...
        if device_requests is not None:
            ...
            if not isinstance(device_requests, list):
                raise host_config_type_error("device_requests", device_requests, "list")
            self["DeviceRequests"] = []
