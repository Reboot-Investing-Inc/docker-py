...
class Container(Model):
    ...
    def exec_run(
        ...
        return ExecResult(
            self.client.api.exec_inspect(resp["Id"])["ExitCode"], exec_output
        )
    ...
    def get_archive(
        self, path, chunk_size=DEFAULT_DATA_CHUNK_SIZE, encode_stream=False
    ):
        """
        ...
        Example:
            ...
            >>> f.close()
        """
        return self.client.api.get_archive(self.id, path, chunk_size, encode_stream)
...
class ContainerCollection(Collection):
    ...
    def run(
        self, image, command=None, stdout=True, stderr=False, remove=False, **kwargs
    ):
        """
        ...
        if detach and remove:
            if version_gte(self.client.api._version, "1.25"):
                ...
                raise RuntimeError(
                    "The options 'detach' and 'remove' cannot "
    ...
    def list(
        ...
        Raises:
            :py:class:`docker.errors.APIError`
                If the server returns an error.
        """
        resp = self.client.api.containers(
...
def _create_container_args(kwargs):
    ...
    if port_bindings:
        ...
        create_kwargs["ports"] = [
            tuple(p.split("/", 1)) for p in sorted(port_bindings.keys())
        ]
    if volumes:
