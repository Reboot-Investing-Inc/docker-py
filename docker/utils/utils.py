    ...
    for k, v in binds.items():
        ...
        if isinstance(v, dict):
            if "ro" in v and "mode" in v:
                raise ValueError(
                    'Binding cannot contain both "ro" and "mode": {}'.format(repr(v))
                )
...
def convert_tmpfs_mounts(tmpfs):
    ...
    if not isinstance(tmpfs, list):
        raise ValueError(
            "Expected tmpfs value to be either a list or a dict, found: {}".format(
                type(tmpfs).__name__
            )
        )
    ...
    for mount in tmpfs:
        if isinstance(mount, str):
            ...
            raise ValueError(
                "Expected item in tmpfs list to be a string, found: {}".format(
                    type(mount).__name__
                )
            )
...
def parse_host(addr, is_win32=False, tls=False):
    ...
    if proto in ("tcp", "ssh"):
        ...
        if port is None or port < 0:
            if proto != "ssh":
                raise errors.DockerException(
                    "Invalid bind address format: port is required:" " {}".format(addr)
                )
            port = 22
...
def format_extra_hosts(extra_hosts, task=False):
    ...
    if task:
        return [f"{v} {k}" for k, v in sorted(iter(extra_hosts.items()))]

    return [f"{k}:{v}" for k, v in sorted(iter(extra_hosts.items()))]
