...
class ImageApiMixin:
    ...
    def import_image(
        ...
        if image or params.get("fromSrc") != "-":  # from image or URL
            ...
            with open(src, "rb") as f:
                return self._result(
                    self._post(u, data=f, params=params, headers=headers, timeout=None)
                )
        else:  # from raw data
            if stream_src:
    ...
    def import_image_from_data(self, data, repository=None, tag=None, changes=None):
        ...
        headers = {"Content-Type": "application/tar"}
        return self._result(
            self._post(u, data=data, params=params, headers=headers, timeout=None)
        )
    ...
    def import_image_from_stream(self, stream, repository=None, tag=None, changes=None):
        return self.import_image(
            src=stream, stream_src=True, repository=repository, tag=tag, changes=changes
        )
