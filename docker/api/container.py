...
class ContainerApiMixin:
    ...
    def attach(
        self, container, stdout=True, stderr=True, stream=False, logs=False, demux=False
    ):
        """
    ...
    def attach_socket(self, container, params=None, ws=False):
        ...
        return self._get_raw_response_socket(
            self.post(
                u,
                None,
                params=self._attach_params(params),
                stream=True,
                headers=headers,
            )
        )
    ...
    def create_container(
        ...
        **Networking**
        ...
        if use_config_proxy:
            environment = (
                self._proxy_configs.inject_proxy_environment(environment) or None
            )
    ...
    @utils.check_resource("container")
    def get_archive(
        self, container, path, chunk_size=DEFAULT_DATA_CHUNK_SIZE, encode_stream=False
    ):
        """
