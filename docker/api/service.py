...
def _check_api_features(
    version, task_template, update_config, endpoint_spec, rollback_config
):
    def raise_version_error(param, min_version):
        raise errors.InvalidVersion(
            "{} is not supported in API version < {}".format(param, min_version)
        )
...
class ServiceApiMixin:
    ...
    def create_service(
        ...
        _check_api_features(
            self._version, task_template, update_config, endpoint_spec, rollback_config
        )
    ...
    def service_logs(
        ...
        if is_tty is None:
            is_tty = self.inspect_service(service)["Spec"]["TaskTemplate"][
                "ContainerSpec"
            ].get("TTY", False)
    ...
    def update_service(
        ...
        if fetch_current_spec:
            ...
            current = self.inspect_service(service, insert_defaults=inspect_defaults)[
                "Spec"
            ]
